locals {
  hsrp_interfaces_map = { for device in local.devices : device.name =>
    { for int in local.hsrp_interfaces : int.id => {
      ...
      groups = length(try(int.hsrp.groups, [])) > 0 ? { for group in try(int.hsrp.groups, []) : "${group.id};${group.address_family}" => {
...
resource "nxos_hsrp" "hsrp" {
  ...
  interfaces = length(local.hsrp_interfaces_map[each.key]) > 0 ? local.hsrp_interfaces_map[each.key] : null
