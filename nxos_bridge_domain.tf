resource "nxos_bridge_domain" "bridge_domain" {
  ...
  bridge_domains = { for vlan in try(local.device_config[each.key].vlans, []) : "vlan-${vlan.id}" => {
    ...
    admin_state  = try(vlan.admin_state, local.defaults.nxos.devices.configuration.vlans.admin_state, null) == null ? null : try(vlan.admin_state, local.defaults.nxos.devices.configuration.vlans.admin_state) ? "active" : "suspend"
    ...
    mac_packet_classify = try(vlan.mac_packet_classify, local.defaults.nxos.devices.configuration.vlans.mac_packet_classify, null) == null ? null : try(vlan.mac_packet_classify, local.defaults.nxos.devices.configuration.vlans.mac_packet_classify) ? "enable" : "disable"
    ...
    cross_connect       = try(vlan.cross_connect, local.defaults.nxos.devices.configuration.vlans.cross_connect, null) == null ? null : try(vlan.cross_connect, local.defaults.nxos.devices.configuration.vlans.cross_connect) ? "enable" : "disable"
  } }
