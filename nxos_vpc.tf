...
resource "nxos_vpc" "vpc" {
  ...
  peer_ip                                      = try(local.device_config[each.key].vpc.virtual_peer_link_destination, local.defaults.nxos.devices.configuration.vpc.virtual_peer_link_destination, null)
  ...
  virtual_ip                                   = try(local.device_config[each.key].vpc.virtual_peer_link_source, local.defaults.nxos.devices.configuration.vpc.virtual_peer_link_source, null)
  ...
  exclude_svi                                  = try(local.device_config[each.key].vpc.dual_active_exclude_interface_vlan, local.defaults.nxos.devices.configuration.vpc.dual_active_exclude_interface_vlan, null)
  mac_bpdu_source_version_2                    = try(local.device_config[each.key].vpc.mac_address_bpdu_source_version_2, local.defaults.nxos.devices.configuration.vpc.mac_address_bpdu_source_version_2, null)
  ...
  keepalive_flush_timeout                      = try(local.device_config[each.key].vpc.keepalive.hold_timeout, local.defaults.nxos.devices.configuration.vpc.keepalive.hold_timeout, null)
