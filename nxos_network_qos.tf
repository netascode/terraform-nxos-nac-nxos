resource "nxos_network_qos" "network_qos" {
  ...
  class_maps = { for cm in try(local.device_config[each.key].qos.network_class_maps, []) : cm.name => {
    match_type = try(cm.match_type, null)
    cos_values = { for cos in try(cm.match_cos_values, []) : cos => {
    } }
  } }
  policy_maps = { for pm in try(local.device_config[each.key].qos.network_policy_maps, []) : pm.name => {
