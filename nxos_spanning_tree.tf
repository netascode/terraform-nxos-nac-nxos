resource "nxos_spanning_tree" "spanning_tree" {
  ...
  control                  = try(local.device_config[each.key].spanning_tree.stateful_ha, local.defaults.nxos.devices.configuration.spanning_tree.stateful_ha, null) != null ? (try(local.device_config[each.key].spanning_tree.stateful_ha, local.defaults.nxos.devices.configuration.spanning_tree.stateful_ha) ? "normal,stateful-ha" : "normal") : null
  ...
  interfaces = merge(
    { for int in try(local.device_config[each.key].interfaces.ethernets, []) : "eth${int.id}" => {
      ...
      priority    = try(int.spanning_tree.priority, local.defaults.nxos.devices.configuration.interfaces.ethernets.spanning_tree.priority, null)
