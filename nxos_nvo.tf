resource "nxos_nvo" "nvo" {
  ...
  nve_interfaces = { for nve_id in try(local.device_config[each.key].interfaces.nve, null) != null ? ["1"] : [] : nve_id => {
    ...
    multisite_source_interface         = try(local.device_config[each.key].interfaces.nve.multisite_source_interface_type, local.defaults.nxos.devices.configuration.interfaces.nve.multisite_source_interface_type, null) != null ? "${local.intf_prefix_map[try(local.device_config[each.key].interfaces.nve.multisite_source_interface_type, local.defaults.nxos.devices.configuration.interfaces.nve.multisite_source_interface_type)]}${try(local.device_config[each.key].interfaces.nve.multisite_source_interface_id, local.defaults.nxos.devices.configuration.interfaces.nve.multisite_source_interface_id, "")}" : null
