resource "nxos_access_list" "access_list" {
  ...
  access_lists = { for acl in try(local.device_config[each.key].ip_access_lists, []) : acl.name => {
    ...
    entries = { for entry in try(acl.entries, []) : entry.sequence_number => {
      ...
      source_prefix             = try(entry.source_prefix, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_prefix, null)
      source_prefix_length      = try(entry.source_prefix_length, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_prefix_length, null)
      source_prefix_mask        = try(entry.source_prefix_mask, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_prefix_mask, null)
      source_address_group      = try(entry.source_address_group, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_address_group, null)
      source_port_operator      = try(entry.source_port_operator, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_port_operator, null)
      source_port_1             = try(tostring(entry.source_port_1), tostring(local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_port_1), null)
      source_port_2             = try(tostring(entry.source_port_2), tostring(local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_port_2), null)
      source_port_group         = try(entry.source_port_group, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source_port_group, null)
      destination_prefix        = try(entry.destination_prefix, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_prefix, null)
      destination_prefix_length = try(entry.destination_prefix_length, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_prefix_length, null)
      destination_prefix_mask   = try(entry.destination_prefix_mask, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_prefix_mask, null)
      destination_address_group = try(entry.destination_address_group, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_address_group, null)
      destination_port_operator = try(entry.destination_port_operator, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_port_operator, null)
      destination_port_1        = try(tostring(entry.destination_port_1), tostring(local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_port_1), null)
      destination_port_2        = try(tostring(entry.destination_port_2), tostring(local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_port_2), null)
      destination_port_group    = try(entry.destination_port_group, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination_port_group, null)
