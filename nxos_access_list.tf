...
resource "nxos_access_list" "access_list" {
  ...
  access_lists = length(try(local.device_config[each.key].ip_access_lists, [])) > 0 ? { for acl in try(local.device_config[each.key].ip_access_lists, []) : acl.name => {
    ...
    entries = length(try(acl.entries, [])) > 0 ? { for entry in try(acl.entries, []) : entry.sequence_number => {
      ...
      source_prefix             = try(entry.source.prefix, null) == "any" ? "0.0.0.0" : try(entry.source.prefix, null)
      source_prefix_length      = try(tostring(entry.source.prefix_length), null)
      source_prefix_mask        = try(entry.source.prefix_mask, null)
      source_address_group      = try(entry.source.addrgroup, null)
      ...
      source_port_group         = try(entry.source.portgroup, null)
      destination_prefix        = try(entry.destination.prefix, null) == "any" ? "0.0.0.0" : try(entry.destination.prefix, null)
      ...
      destination_address_group = try(entry.destination.addrgroup, null)
      ...
      destination_port_group    = try(entry.destination.portgroup, null)
  ...
  ipv6_access_lists = length(try(local.device_config[each.key].ipv6_access_lists, [])) > 0 ? { for acl in try(local.device_config[each.key].ipv6_access_lists, []) : acl.name => {
    ...
    entries = length(try(acl.entries, [])) > 0 ? { for entry in try(acl.entries, []) : entry.sequence_number => {
      ...
      action                    = try(entry.action, null)
      protocol                  = try(tostring(entry.protocol), null)
      source_prefix             = try(entry.source.prefix, null) == "any" ? "::" : try(entry.source.prefix, null)
      source_prefix_length      = try(tostring(entry.source.prefix_length), null)
      source_prefix_mask        = try(entry.source.prefix_mask, null)
      source_address_group      = try(entry.source.addrgroup, null)
      source_port_operator      = try(entry.source.port_operator, null)
      ...
      source_port_2             = try(tostring(entry.source.port_2), null)
      source_port_group         = try(entry.source.portgroup, null)
      destination_prefix        = try(entry.destination.prefix, null) == "any" ? "::" : try(entry.destination.prefix, null)
      destination_prefix_length = try(tostring(entry.destination.prefix_length), null)
      destination_prefix_mask   = try(entry.destination.prefix_mask, null)
      destination_address_group = try(entry.destination.addrgroup, null)
      destination_port_operator = try(entry.destination.port_operator, null)
