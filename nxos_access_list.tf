resource "nxos_access_list" "access_list" {
  ...
  access_lists = { for acl in try(local.device_config[each.key].ip_access_lists, []) : acl.name => {
    ...
    entries = { for entry in try(acl.entries, []) : entry.sequence_number => {
      ...
      source_prefix_length      = try(entry.source.prefix_length, local.defaults.nxos.devices.configuration.ip_access_lists.entries.source.prefix_length, null)
      ...
      destination_prefix_length = try(entry.destination.prefix_length, local.defaults.nxos.devices.configuration.ip_access_lists.entries.destination.prefix_length, null)
