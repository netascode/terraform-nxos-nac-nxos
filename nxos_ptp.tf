locals {
  ptp_interfaces_map = { for device in local.devices : device.name =>
    { for int in local.ptp_interfaces : int.id => {
      announce_interval_type               = try(int.ptp.announce_interval_profile, null) != null ? try({ "smpte-2059-2" = "smpte-2059-2" }[int.ptp.announce_interval_profile], int.ptp.announce_interval_profile) : null
      announce_interval_value              = try(int.ptp.announce_interval, null)
      announce_timeout_type                = try(int.ptp.announce_timeout_profile, null) != null ? try({ "smpte-2059-2" = "smpte-2059-2" }[int.ptp.announce_timeout_profile], int.ptp.announce_timeout_profile) : null
      announce_timeout_value               = try(int.ptp.announce_timeout, null)
      asymmetric_delay_value               = try(int.ptp.asymmetric_delay_value, null)
      asymmetric_direction                 = try(int.ptp.asymmetric_delay_direction, null) != null ? try({ "forward-path" = "forward-path", "reverse-path" = "reverse-path" }[int.ptp.asymmetric_delay_direction], int.ptp.asymmetric_delay_direction) : null
      cost                                 = try(int.ptp.cost, null)
      delay_request_min_interval_type      = try(int.ptp.delay_request_minimum_interval_profile, null) != null ? try({ "smpte-2059-2" = "smpte-2059-2" }[int.ptp.delay_request_minimum_interval_profile], int.ptp.delay_request_minimum_interval_profile) : null
      ...
      receive_no_match                     = try(int.ptp.receive_no_match, null) != null ? try({ "follow-rx" = "follow-rx" }[int.ptp.receive_no_match], int.ptp.receive_no_match) : null
      role                                 = try(int.ptp.role, null)
      sync_interval_type                   = try(int.ptp.sync_interval_profile, null) != null ? try({ "smpte-2059-2" = "smpte-2059-2" }[int.ptp.sync_interval_profile], int.ptp.sync_interval_profile) : null
      ...
      unicast_vrf_ipv6                     = try(int.ptp.ucast_source_vrf_ipv6, null)
      vlan                                 = try(int.ptp.vlan, null) != null ? "vlan-${try(int.ptp.vlan)}" : null
      peers = length(try(int.ptp.ucast_peers, [])) > 0 ? { for peer in try(int.ptp.ucast_peers, []) : peer.ip => {
        negotiation_schema = try(peer.transmission_unicast_negotiation_schema, null)
      } } : null
