resource "nxos_hardware_telemetry" "hardware_telemetry" {
  ...
  device                           = each.key
  sflow_agent_address              = try(local.device_config[each.key].sflow.agent_ip, null)
  ...
  sflow_max_header_size            = try(local.device_config[each.key].sflow.max_sampled_size, null)
