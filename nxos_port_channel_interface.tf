...
resource "nxos_port_channel_interface" "port_channel_interface" {
  ...
  port_channel_interfaces = { for int in try(local.device_config[each.key].interfaces.port_channels, []) : "po${int.id}" => {
    ...
    transmission_mode     = try(int.switchport_transparent_mode, local.defaults.nxos.devices.configuration.interfaces.port_channels.switchport_transparent_mode, null) == null ? null : (try(int.switchport_transparent_mode, local.defaults.nxos.devices.configuration.interfaces.port_channels.switchport_transparent_mode) ? "trans-port" : "not-a-trans-port")
