...
resource "nxos_port_channel_interface" "port_channel_interface" {
  ...
  port_channel_interfaces = { for int in try(local.device_config[each.key].interfaces.port_channels, []) : "po${int.id}" => {
    ...
    trunk_vlans           = !try(int.switchport.enabled, local.defaults.nxos.devices.configuration.interfaces.port_channels.switchport.enabled, true) ? "1-4094" : try(provider::utils::normalize_vlans(try(int.switchport.trunk_allowed_vlans, local.defaults.nxos.devices.configuration.interfaces.port_channels.switchport.trunk_allowed_vlans), "string-nxos"), null)
